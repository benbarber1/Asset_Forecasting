"""
Van Westendorp PSM — OUS (Outside US) Only
===========================================
Reads the last 20 columns (UK, FR, IT, ES, DE) from the raw data file
and applies the same currency conversions as the main script.

Column layout within the 20 OUS columns (0-based):
  UK (0–3) | FR (4–7) | IT (8–11) | ES (12–15) | DE (16–19)

Currency conversions applied:
  - UK  (cols 0–3):   USD → GBP  ×1.36612
  - DE  (cols 16–19): USD → EUR  ×1.173709

  NOTE: If FR, IT, ES also need USD→EUR conversion, change
        EUR_COLS to slice(4, 20) in the CONFIGURATION block below.

Outputs:
  - Console:           4 VW price intersection points
  - vw_ous_output.xlsx Chart-ready cumulative % table + Price Points summary
  - vw_ous_chart.png   PSM plot
"""
...
OUTPUT_EXCEL = r"C:\Users\bbarber\OneDrive - BGB Group\Documents\1) Project work\BD\BD-014\Conjoint Analysis\17 June\vw_ous_output.xlsx"
OUTPUT_CHART = r"C:\Users\bbarber\OneDrive - BGB Group\Documents\1) Project work\BD\BD-014\Conjoint Analysis\17 June\vw_ous_chart.png"
CHART_TITLE  = "Van Westendorp PSM — OUS (UK / FR / IT / ES / DE)"
CURRENCY_SYM = "$"   # OUS values are a mix; $ is used as a generic label

USD_TO_GBP   = 1.36612
USD_TO_EUR   = 1.173709

# Slices within the 20-column OUS block (0-based)
GBP_COLS     = slice(0, 4)    # UK
EUR_COLS     = slice(16, 20)  # DE only — change to slice(4, 20) for FR+IT+ES+DE

# Column order within each market's 4-column block
QUESTIONS    = ["Too Expensive", "Expensive", "Cheap", "Too Cheap"]
OUS_MARKET_STARTS = [0, 4, 8, 12, 16]   # UK, FR, IT, ES, DE within the 20 OUS columns


# ─────────────────────────────────────────────
#  STEP 1 — Load raw data (skip 2 header rows, OUS cols only)
# ─────────────────────────────────────────────
def load_data(path: str) -> pd.DataFrame:
    print(f"Loading: {path}  [OUS only — columns 5–24]")
    if path.endswith(".csv"):
        raw = pd.read_csv(path, header=None, skiprows=2)
    else:
        raw = pd.read_excel(path, header=None, skiprows=2)

    raw = raw.iloc[:, 4:24].copy()        # columns 5–24 (0-based: 4–23)
    raw = raw.reset_index(drop=True)
    raw.columns = range(20)               # re-index to 0–19 for easy slicing
...
def convert_currencies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.round(0)
    print("  Currency conversion applied (UK→GBP, DE→EUR).")
    return df


# ─────────────────────────────────────────────
#  STEP 3 — Stack 5 OUS markets into 4 question columns
# ─────────────────────────────────────────────
def stack_markets(df: pd.DataFrame) -> dict[str, np.ndarray]:
    stacked = {}
    for q_idx, question in enumerate(QUESTIONS):
        cols = [start + q_idx for start in OUS_MARKET_STARTS]
        combined = pd.concat(
...
def main():
    ...
    print("\nStacking OUS markets into 4 question columns...")
    ...
    print("\n" + "=" * 48)
    print("  VAN WESTENDORP PRICE POINTS — OUS")
    print("=" * 48)
